...
SProgram::SProgram(OpenGLContext *context)
...
void SProgram::create(const char *vertfile, const char *fragfile)
{
    ...
    const char* vs = vertSource;
    const char* fs = fragSource;

    // Send the shader text to OpenGL and store it in the shaders specified by the handles vertShader and fragShader
    context->glShaderSource(vertShader, 1, &vs, 0);
    context->glShaderSource(fragShader, 1, &fs, 0);
...
char* SProgram::textFileRead(const char* fileName) {
    char* text;
...
QString SProgram::qTextFileRead(const char *fileName)
{
    QString text;
    QFile file(fileName);
...
void SProgram::printShaderInfoLog(int shader)
{
    int infoLogLen = 0;
    int charsWritten = 0;
    GLchar *infoLog;
...
void SProgram::printLinkInfoLog(int prog)
{
    int infoLogLen = 0;
    int charsWritten = 0;
    GLchar *infoLog;
